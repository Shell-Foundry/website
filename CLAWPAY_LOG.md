# ClawPay Project Log
## Started: 2026-01-30 as PayClawd
## Rebranded: 2026-01-30 to ClawPay
## Parent Company Rebrand: 2026-01-30 to ShellFoundry

### BRAND
- **Product Name:** ClawPay
- **Parent Company:** ShellFoundry 🐚
- **Tagline:** Payment infrastructure for OpenClaw skills

### FOUNDER PRESENCE
- **X/Twitter:** @ShellFoundry
- **Reddit:** u/ShellFoundry
- **Domain:** ShellFoundry.com (purchased)
- **Email:** dev@ShellFoundry.com (pending setup)

### LIVE URLS
- **Dashboard:** https://clawpay-dashboard.vercel.app
- **Contract (Testnet):** https://sepolia.basescan.org/address/0x72CaF410E276c50e2f5a5C76f9348242c66bE86c

### COMPLETED ✅
1. ✅ Smart contract (ClawPaySplitter.sol) deployed on Base Sepolia
2. ✅ ClawPay skill integration (skill-clawpay/)
3. ✅ Developer dashboard deployed
4. ✅ Payment flow tested (10%/90% split works)
5. ✅ Rebrand complete (PayClawd → ClawPay)
6. ✅ All documentation updated

### PENDING REQUESTS
1. ⏳ Set up shellfoundry.com website (GoDaddy)
2. ⏳ Set up dev@shellfoundry.com email
3. ⏳ Deploy to mainnet ($20-50)
4. ⏳ Find pilot developers
5. ⏳ Submit to OpenClaw Hub

### CONTRACT DETAILS
- **Address:** 0x72CaF410E276c50e2f5a5C76f9348242c66bE86c
- **Network:** Base Sepolia Testnet
- **Owner:** 0xD49533AE308c190e7823f1602ED58C9c8e02613f
- **Fee Split:** 10% platform / 90% developer

### FILES LOCATION
All project files: `/root/clawd/`
- contracts/ - Smart contract code
- skill-clawpay/ - Skill integration
- dashboard/ - Web dashboard
- *.md - Documentation
